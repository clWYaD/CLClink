...
@class CLCoreStatement;

/** Represents the results of executing a query on an `<CLCoreDatabase>`.
...
@interface CLCoreResultSet : NSObject

@property (nonatomic, retain, nullable) CLCoreDatabase *parentDB;
...
/** `CLCoreStatement` used by result set. */

@property (atomic, retain, nullable) CLCoreStatement *statement;

///------------------------------------
/// @name Creating and closing a result set
///------------------------------------

/** Create result set from `<CLCoreStatement>`
 
 @param statement A `<CLCoreStatement>` to be performed
 
 @param aDB A `<CLCoreDatabase>` to be used
 
 @return A `CLCoreResultSet` on success; `nil` on failure
 */

+ (instancetype)resultSetWithStatement:(CLCoreStatement *)statement usingParentDatabase:(CLCoreDatabase*)aDB;
